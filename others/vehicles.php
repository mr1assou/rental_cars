...
<body>
    ...
    <!-- sidebar -->
            <div class="w-[20%] bg-black h-full fixed  top-0 mt-20 opacity-70 -z-50 "> 
                 <div class="py-2 w-full px-2 ">
                    <a href="./vehicles.php?location=<?php echo $_GET['location'];?>&pick_up_date=<?php echo $pick;?>&return_date=<?php echo $return;?>" class="mt-2 inline-block w-full bg-orange text-white p-2 text-sm transition duration-300 ease-in-out hover:bg-white hover:text-orange text-center cursor-pointer">See All Vehicles</a>       
                </div>
    ...
    <!-- content -->
            <div class="w-[80%]  h-[100%] flex flex-wrap justify-evenly container" style="margin-left: 270px;">
                <?php
                if($count==1){
                    displayForGuest($connect,$city,1);
                }
                ?>
            </div>
        </div>
